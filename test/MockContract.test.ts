...
describe("MockContract", function () {
  ...
  describe("givenAnyRevertWithMessage", function () {
    ...
    describe("givenCalldataReturn", function () {
      it("should return the mocked value", async function () {
        ...
        const callData = complexInterface.interface.encodeFunctionData("acceptAddressUintReturnBoolView", [
          ZeroAddress,
          10,
        ]);
        ...
        expect(
          await complexInterface.acceptAddressUintReturnBoolView("0x0000000000000000000000000000000000000001", 10),
        ).equal(false);
        ...
        const secondAlternativeCallData = complexInterface.interface.encodeFunctionData("acceptUintReturnAddressView", [
          7,
        ]);
        const signerAddress = signers[0].address;
  ...
  describe("givenCalldataRevert", function () {
    it("should revert if mocked", async function () {
      ...
      const callData = await complexInterface.interface.encodeFunctionData("acceptAddressUintReturnBoolView", [
        ZeroAddress,
        10,
      ]);
  ...
  describe("givenMethodRevert", function () {
    it("should revert if mocked", async function () {
      ...
      await expect(
        complexInterface.acceptAddressUintReturnBoolView("0x0000000000000000000000000000000000000000", 10),
      ).to.be.revertedWith("");
      await expect(
        complexInterface.acceptAddressUintReturnBoolView("0x0000000000000000000000000000000000000001", 12),
      ).to.be.revertedWith("");
  ...
  describe("givenMethodRevertWithMessage", function () {
    it("should revert if mocked and return message", async function () {
      ...
      await expect(
        complexInterface.acceptAddressUintReturnBoolView("0x0000000000000000000000000000000000000001", 12),
      ).to.be.revertedWith("This is Sparta!!!");
  ...
  describe("test mock priority", function () {
    ...
    const testSpecificMocks = async function (
      ...
      await mockContract.givenCalldataReturn(
        callData,
        ethers.AbiCoder.defaultAbiCoder().encode(["string"], ["return specific"]),
      );
    ...
    it("all specific mocks should be prioritized over return any mock", async function () {
      ...
      // MethodId mock set
      await mockContract.givenMethodReturn(
        methodId,
        ethers.AbiCoder.defaultAbiCoder().encode(["string"], ["methodId"]),
      );
